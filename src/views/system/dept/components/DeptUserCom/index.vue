<template>
  ...
  <fs-crud
    ...
    :customClass="
      !showCount
        ? 'dept-user-com-box dept-user-com-table'
        : 'dept-user-com-box dept-user-com-table-cover'
    "
  >
    <template #toolbar-left>
      <el-button
        :icon="!showCount ? 'Hide' : 'View'"
        circle
        @click="showCount = !showCount"
      ></el-button>
    </template>
    <template #actionbar-right>
      <importExcel api="api/system/user/" v-auth="'user:Import'"
        >导入
      </importExcel>
    </template>
  </fs-crud>
...
const initDeptCountBarChart = () => {
  ...
  const yAxisData =
    deptInfo.value.sub_dept_map?.map((item) => item.count) || [];
...
const getDeptInfo = async () => {
  const res = await getDeptInfoById(
    currentDeptId.value,
    isShowChildFlag.value ? "1" : "0"
  );
  if (res?.code === 2000) {
