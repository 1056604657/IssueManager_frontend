  ...
  setup() {
    ...
    const initLineChart = () => {
      ...
      const option = {
        ...
        legend: { data: ["新增issue", "解决issue"], right: 0 },
        xAxis: {
          boundaryGap: false,
          data: state.daily_count.map((item) => item.date),
        },
        yAxis: [
          {
            type: "value",
        ...
        series: [
          {
            ...
            symbol: "circle",
            smooth: true,
            data: state.daily_count.map((item) => item.created_count),
          ...
          },
          {
            ...
            symbol: "circle",
            smooth: true,
            data: state.daily_count.map((item) => item.resolved_count),
    ...
    const initPieChart = () => {
      ...
      var getname = state.unresolved_project_count.map(
        (item) => item.project_name
      );
      var getvalue = state.unresolved_project_count.map(
        (item) => item.unresolve_count
      );
      var data = [];
      ...
      const option = {
        ...
        tooltip: { trigger: "item", formatter: "{b}:{c}个 <br/> {d}%" },
        graphic: {
          elements: [
            {
              type: "image",
